...
import { seedPointsTable } from "../services/pointsTableService.js";
...
export const update = asyncHandler(async (req, res) => {
  ...
  if (req.body.status === "completed") {
    await Match.updateMany(
      { tournamentId: t._id, status: { $nin: ["completed", "abandoned"] } },
      { $set: { status: "abandoned", "result.text": "Match Abandoned (Tournament Ended)" } }
    );
  }
  await t.save();
  await seedPointsTable(t._id);
...
export const pointsTable = asyncHandler(async (req, res) => {
  ...
  // Build a complete table including teams with 0 played
  const tableMap = new Map(
    (t.pointsTable || []).map((row) => [
      String(row.teamId?._id || row.teamId),
      row,
    ]),
  );

  const fullTable = t.teams.map((team) => {
    const row = tableMap.get(String(team._id));
    return row
      ? { ...row.toObject(), teamId: team }
      : {
          teamId: team,
          played: 0,
          won: 0,
  ...
  fullTable.sort((a, b) => b.points - a.points || b.nrr - a.nrr);
  res.json(fullTable);
});
